...
def generate_txt_report(domain, subdomains):
	txt_report = "\rSubdomain Enumeration Results for " + str(domain)
	seperator = "=" * (len(txt_report) - 1)
	txt_report += "\n" + seperator + "\n"
	for subdomain in subdomains:
		txt_report += str(subdomain) + "\n"
	txt_report += seperator + "\n"
	return txt_report


def passif_scan(domain, get_request):
	subdomains = []
	...
	txt_report = generate_txt_report(domain, clean_subdomains)
	return txt_report